...
// ------------------------------------------------------------
// [페이징 로직 시작] - 자료실(board)과 동일한 로직 적용
// ------------------------------------------------------------
$list_size = 10; // 한 페이지에 보여줄 게시글 수
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
if ($page < 1) $page = 1;

// 전체 게시글 수 조회 (notice 테이블)
...
// 2. 공지사항 데이터 조회 (중요공지 우선 정렬 + LIMIT 적용)
$query = "SELECT * FROM notice 
          ORDER BY (CASE WHEN is_notice = 'Y' THEN 1 ELSE 2 END) ASC, reg_date DESC, idx DESC 
...
<head>
    ...
    <style>
        ...
        /* 중요 공지 배지 */
        .badge_notice { padding: 3px 8px; background: #FF5D27; color: #fff; border-radius: 3px; font-size: 11px; font-weight: 700; margin-right: 5px; vertical-align: middle; }
...
<div class="admin_content">
    <div class="admin_title_area">
        <h2>공지사항 관리</h2>
        <a href="notice_form.php" class="btn_add">+ 새 공지 등록</a>
    </div>
                ...
                <tr>
                    ...
                    <td class="txt_left">
                        <?php if($row['is_notice'] == 'Y'): ?>
                            <span class="badge_notice">중요</span>
                        <?php endif; ?>
                        <strong><?php echo htmlspecialchars($row['n_title_ko']); ?></strong>
                        <div class="lang_status">
                            <span class="lang_dot on">KO</span>
                            <span class="lang_dot <?php echo !empty($row['n_title_en']) ? 'on' : ''; ?>">EN</span>
                            <span class="lang_dot <?php echo !empty($row['n_title_zh']) ? 'on' : ''; ?>">ZH</span>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <a href="notice_form.php?idx=<?php echo $row['idx']; ?>" class="btn_edit">수정</a>
                        <a href="notice_db.php?mode=delete&idx=<?php echo $row['idx']; ?>" class="btn_del" onclick="return confirm('정말 삭제하시겠습니까? 파일도 함께 삭제됩니다.')">삭제</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr>
                    <td colspan="5" class="no_data">등록된 공지사항이 없습니다.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
