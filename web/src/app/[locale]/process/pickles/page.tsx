    ...
    <>
      <main id="sub_contents" className="process_page">
        <SubVisual
          ...
          currentLabel={t('sub_proc_pickles')}
          title={t('sub_proc_pickles')}
          ...
          <div className="process_inner">
            <div className="process_top_info">
              <div className="tit_group">
                <span className="sub_tit">{t('proc_pickles_top_sub')}</span>
                <h3 className="main_tit">{t('proc_pickles_top_main')}</h3>
              </div>
              <p className="desc_txt">{nl2br(t('proc_pickles_top_desc'))}</p>
            </div>
              ...
              {STEPS.map((num) => (
                <div className="step_item" key={num}>
                  ...
                  <div className="step_info">
                    <h4 className="step_tit">{t(`proc_step${num}_tit`)}</h4>
                    <p className="step_desc">{nl2br(t(`proc_step${num}_desc`))}</p>
                  </div>
                  <div
                    className="step_img"
                    style={{ backgroundImage: `url('/images/sub/01-${num}.png')` }}
                  ></div>
                </div>
              ))}
            </div>
      ...
      <Script id="process-pickles-reveal" strategy="afterInteractive">
        {`
const __init = () => {
